...
#include <gtsam/inference/JunctionTree-inst.h>  // We need the inst file because we'll make a special JT templated on ISAM2Copy
...
/* ************************************************************************* */
// Special BayesTree class that uses ISAM2Copy cliques - this is the result of
// reeliminating ISAM2Copy subtrees.
class ISAM2CopyBayesTree : public ISAM2Copy::Base {
 public:
  typedef ISAM2Copy::Base Base;
  typedef ISAM2CopyBayesTree This;
...
/* ************************************************************************* */
// Special JunctionTree class that produces ISAM2Copy BayesTree cliques, used for
// reeliminating ISAM2Copy subtrees.
class ISAM2CopyJunctionTree
    : public JunctionTree<ISAM2CopyBayesTree, GaussianFactorGraph> {
 public:
  typedef JunctionTree<ISAM2CopyBayesTree, GaussianFactorGraph> Base;
  typedef ISAM2CopyJunctionTree This;
  typedef boost::shared_ptr<This> shared_ptr;

  size_t getSize(const sharedNode& node) const {
    size_t subtree_size = 1;
    for (const sharedNode& child : node->children) {
      subtree_size += getSize(child);
    }
    return subtree_size;
  }
  ...
  size_t size() const {
    size_t tree_size = 0;
    for (const sharedNode& node : roots_) {
      tree_size += getSize(node);
    }
    return tree_size;
  }

  explicit ISAM2CopyJunctionTree(const GaussianEliminationTree& eliminationTree)
...
struct GTSAM_EXPORT DeltaImpl {
  struct GTSAM_EXPORT PartialSolveResult {
    ISAM2Copy::sharedClique bayesTree;
  };
   ...
   */
  static size_t UpdateGaussNewtonDelta(const ISAM2Copy::Roots& roots,
   ...
   */
  static size_t UpdateRgProd(const ISAM2Copy::Roots& roots,
                             const KeySet& replacedKeys,
...
struct GTSAM_EXPORT UpdateImpl {
  const ISAM2CopyParams& params_;
  const ISAM2CopyUpdateParams& updateParams_;
  UpdateImpl(const ISAM2CopyParams& params, const ISAM2CopyUpdateParams& updateParams)
  ...
  static void LogStartingUpdate(const NonlinearFactorGraph& newFactors,
                                const ISAM2Copy& isam2) {
    gttic(pushBackFactors);
    const bool debug = ISDEBUG("ISAM2Copy update");
    const bool verbose = ISDEBUG("ISAM2Copy update verbose");

    if (verbose) {
      std::cout << "ISAM2Copy::update\n";
      isam2.print("ISAM2Copy: ");
    }
  ...
  void updateKeys(const KeySet& markedKeys, ISAM2CopyResult* result) const {
    gttic(updateKeys);
  ...
  static void CheckRelinearizationRecursiveMap(
      ...
      const ISAM2Copy::sharedClique& clique, KeySet* relinKeys) {
  ...
  static void CheckRelinearizationRecursiveDouble(
    ...
    for (Key var : *clique->conditional()) {
      ...
      if (maxDelta >= threshold) {
        // std::cout << "isam2 maxDelta: " << maxDelta << "\n";
        relinKeys->insert(var);
        relinearize = true;
      }
    }
  ...
  static KeySet CheckRelinearizationPartial(
      const ISAM2Copy::Roots& roots, const VectorValues& delta,
      const ISAM2CopyParams::RelinearizationThreshold& relinearizeThreshold) {
    KeySet relinKeys;
    for (const ISAM2Copy::sharedClique& root : roots) {
      if (relinearizeThreshold.type() == typeid(double))
      {
        // std::cout << "CheckRelinearizationPartial: " << boost::get<double>(relinearizeThreshold) << "\n";
  ...
  static KeySet CheckRelinearizationFull(
    ...
    if (const double* threshold = boost::get<double>(&relinearizeThreshold)) {
      std::cout << "CheckRelinearizationFull: " << threshold << "\n";
  ...
  KeySet gatherRelinearizeKeys(const ISAM2Copy::Roots& roots,
                               const VectorValues& delta,
  ...
  void recordRelinearizeDetail(const KeySet& relinKeys,
                               ISAM2CopyResult::DetailedResults* detail) const {
  ...
  // their ancestors.
  void findFluid(const ISAM2Copy::Roots& roots, const KeySet& relinKeys,
                 KeySet* markedKeys,
                 ISAM2CopyResult::DetailedResults* detail) const {
    gttic(findFluid);
  ...
  static void LogRecalculateKeys(const ISAM2CopyResult& result) {
  ...
  static GaussianFactorGraph GetCachedBoundaryFactors(
      const ISAM2Copy::Cliques& orphans) {
